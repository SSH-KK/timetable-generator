		...
		if (event.currentTarget.dataset) {
			...
			if (dataset.dayid && dataset.eventid && dataset.groupid && dataset.lessonnum) {
				...
				console.log(
					dayid,
					eventid,
					parseInt(classNum.replace("lessons", "")),
					groupid,
					isPair,
	...
	const splittoggle = (event: React.MouseEvent<HTMLButtonElement>) => {
		if (event.currentTarget.dataset) {
			...
			if (dataset.dayid && dataset.eventid && dataset.groupid) {
				...
				if(days[dayid].events[eventid][classNum][0][groupid] == days[dayid].events[eventid][classNum][1][groupid]){
					setCardSelectionState((prev)=>prev.map((day, dayIndex)=>dayIndex == dayid ? day.map((event, eventIndex)=>eventIndex == eventid ? event.map((group, groupIndex)=>groupIndex == groupid ? !group : group) : event) : day))
				}
						...
						event[classNum][0].map((_, cardID) => (
							<div className="col-2 border border-1 py-5 px-2 position-relative">
								{cardSelectionState[dayIndex][eventIndex]
									...
									: ""}
									<button onClick={splittoggle} disabled={event[classNum][0][cardID] != event[classNum][1][cardID]} data-dayId={dayIndex} data-eventId={eventIndex} data-groupId={cardID} className={`btn btn-secondary ${styles.splittoggle}`}>
										<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-chevron-bar-expand" viewBox="0 0 16 16">
