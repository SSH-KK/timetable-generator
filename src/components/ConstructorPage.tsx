...
const ConstructorPage: React.FC<ConstructorPageProps> = ({ days, subjects, cards, classNum, addButton }) => {
	return (
		<>
			{
				days.map((day, dayIndex) => (
					<div className='row' key={dayIndex}>
						<div className='col-12 text-center border-bottom border-1'>
							<h4>{`${new Date(day.date).getDate()} ${assets.weekdays[new Date(day.date).getDay() - 1 != -1 ? new Date(day.date).getDay() - 1 : 6]}`}</h4>
						</div>
