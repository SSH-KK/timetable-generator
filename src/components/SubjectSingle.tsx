...
const SubjectSingle: React.FC<SubjectSingleProps> = ({subject, addTeacher, subjectId})=>{
		...
		<div className="col-11 border rounded rounded-2 border-3 p-2 mt-2">
			...
			<div className="d-flex flex-wrap">
				{
					subject.teachers.map((teacher, teacherIndex)=>
						<div className="p-1 rounded rounded-3 me-2 shadow-sm">
							...
							<button type="button" className="btn-close h-0" aria-label="Close"></button>
						</div>
					)
				}
			</div>
			<form onSubmit={addTeacherForm} className="mt-2">
				<div className="input-group input-group-sm">
				  <input type="text" onChange={inputChange} value={inputState} className="form-control border border-3 rounded rounded-3" placeholder="Преподаватель"/>
				</div>
			</form>
		</div>
	)
