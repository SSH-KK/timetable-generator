...
const generateDocument = (
  ...
  const sheet = workbook.addWorksheet('Расписание',{properties:{
    defaultColWidth:25,
    defaultRowHeight: 55
  }})
  assets.mainHeader.forEach((ob, obIndex)=>{
    ...
    sheet.getCell(2,4+2*obIndex).value = `${getGroupNumber(generation)}${2*obIndex+1} группа`
    sheet.getCell(2,4+2*obIndex).alignment = assets.baseAlignment
    sheet.getCell(2,4+2*obIndex+1).value = `${getGroupNumber(generation)}${2*obIndex+2} группа`
  ...
  timetableState.days.forEach((day, dayIndex)=>{
    ...
    day.events.forEach((event, eventIndex)=>{
      ...
      const lesNumCells = [sheet.getCell(4+celsUp+2*eventIndex,2), sheet.getCell(5+celsUp+2*eventIndex,2)]
      const timeCells = [sheet.getCell(4+celsUp+2*eventIndex,3), sheet.getCell(5+celsUp+2*eventIndex,3)]
      for(let i=0; i<2; i++){
      ...
      event[`lessons${classNumber}` as LessonsType][0].forEach((par, parIndex)=>{
        if(parIndex%2==0){
          ...
          if(allSame == 4){
            sheet.mergeCells(4+celsUp+2*eventIndex,4+2*Math.trunc(parIndex/2),5+celsUp+2*eventIndex,5+2*Math.trunc(parIndex/2))
          }
          else{
            if(temp_cells[0][1] == temp_cells[2][1] && temp_cells[0][1]!=-1){
              sheet.mergeCells(4+celsUp+2*eventIndex,4+2*Math.trunc(parIndex/2),5+celsUp+2*eventIndex,4+2*Math.trunc(parIndex/2))
            }
            else if(temp_cells[0][1] == temp_cells[1][1] && temp_cells[0][1]!=-1){
              sheet.mergeCells(4+celsUp+2*eventIndex,4+2*Math.trunc(parIndex/2),4+celsUp+2*eventIndex,5+2*Math.trunc(parIndex/2))
            }
            else if(temp_cells[2][1] == temp_cells[3][1] && temp_cells[1][1]!=-1){
              sheet.mergeCells(5+celsUp+2*eventIndex,4+2*Math.trunc(parIndex/2),5+celsUp+2*eventIndex,5+2*Math.trunc(parIndex/2))
            }
            else if(temp_cells[1][1] == temp_cells[3][1] && temp_cells[1][1]!=-1){
              sheet.mergeCells(4+celsUp+2*eventIndex,5+2*Math.trunc(parIndex/2),5+celsUp+2*eventIndex,5+2*Math.trunc(parIndex/2))
            }
          }
        }
      })
...
const createDocument = (
  ...
  generateDocument(classNumber, generation, timetableState, firstDay)
  .then((buffer)=>{
